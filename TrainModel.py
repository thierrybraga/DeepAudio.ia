...
import pickle # IMPORTANTE: Adicione esta linha para usar o módulo pickle
...
class ModelTrainer:
    ...
    def __init__(self,
                 ...
                 label_encoder: Optional[LabelEncoder] = None # ADIÇÃO: Parâmetro para receber o LabelEncoder
        ...
        Args:
            ...
            label_encoder: A instância do LabelEncoder ajustada, se as labels forem strings.
        ...
        self.label_encoder = label_encoder # ADIÇÃO: Armazena o LabelEncoder passado no construtor
    ...
    def train_model(self, X: np.ndarray, y: np.ndarray) -> Optional[tf.keras.callbacks.History]:
        ...
        if y.dtype == object:
            ...
            # Se o LabelEncoder não foi fornecido no construtor, crie e ajuste-o aqui.

            # Se já foi fornecido, ele já deve estar ajustado.
            ...
            if self.label_encoder is None:
                ...
                # Se o LabelEncoder foi fornecido, use-o para transformar os dados.

                # É crucial que ele já tenha sido `fit` em todos os rótulos possíveis ANTES de ser passado.
                ...
                try:
                    ...
                    logger.error(f"Erro ao transformar labels com LabelEncoder fornecido: {e}. Verifique se o encoder foi ajustado corretamente.")
        ...
        try:
            ...
            # ADIÇÃO: Salvar o LabelEncoder após o treinamento bem-sucedido

            if self.label_encoder is not None:

                # Gera um nome de arquivo para o LabelEncoder com o mesmo timestamp do modelo .h5
                ...
                try:
                    ...
                    logger.error(f"Erro ao salvar o LabelEncoder em {encoder_path}: {e}")

            else:

                logger.warning("LabelEncoder não foi ajustado ou fornecido. Não será salvo.")
...
if __name__ == "__main__":
    ...
    dummy_y_text = np.array(['REAL'] * int(num_samples * 0.75) + ['FAKE'] * int(num_samples * 0.25))

    np.random.shuffle(dummy_y_text)



    print(f"Dummy X shape: {dummy_X.shape}")

    print(f"Dummy y shape: {dummy_y_text.shape}")

    print(f"Distribuição das classes no dummy_y: {np.unique(dummy_y_text, return_counts=True)}")



    # ADIÇÃO: Inicializar e ajustar o LabelEncoder AQUI, antes de passá-lo ao ModelTrainer

    label_encoder_instance = LabelEncoder()

    dummy_y_encoded = label_encoder_instance.fit_transform(dummy_y_text)

    num_classes = len(label_encoder_instance.classes_)



    print(f"Labels codificadas (primeiros 5): {dummy_y_encoded[:5]}")

    print(f"Classes reconhecidas pelo LabelEncoder: {label_encoder_instance.classes_}")
    ...
    # O LabelEncoder codifica em ordem alfabética por padrão, então 'FAKE' (0) e 'REAL' (1)

    unique_classes_encoded, class_counts = np.unique(dummy_y_encoded, return_counts=True)

    total_samples_for_weights = len(dummy_y_encoded)

    calculated_class_weights = {

        cls: total_samples_for_weights / (num_classes * count)

        for cls, count in zip(unique_classes_encoded, class_counts)
    ...
    for arch in architectures_to_test:
        ...
        temp_dummy_X = dummy_X # Valor padrão, pode ser ajustado abaixo

        temp_dummy_y = dummy_y_encoded # Usar as labels já codificadas



        # Ajustar a forma de entrada para o Transformer, se aplicável

        if arch == "transformer":

            # Para transformer, geralmente (sequence_length, feature_dim) é esperado após o processamento inicial.

            # Se a entrada original é (frames, features, 1), podemos simplificá-la para (frames, features).

            current_dummy_input_shape = (dummy_input_shape[0], dummy_input_shape[1])

            # Se os dados dummy são 4D, garantir que correspondam à expectativa 3D para o transformer antes de passar
        ...
        trainer = ModelTrainer(
            ...
            input_shape=current_dummy_input_shape, # Usar o shape ajustado

            num_classes=num_classes,

            epochs=5,  # Reduzido para teste rápido

            batch_size=16,  # Reduzido para teste rápido

            patience=3,  # Reduzido para teste rápido
            ...
            label_encoder=label_encoder_instance # ADIÇÃO: Passa a instância do LabelEncoder aqui
        ...
        try:
            ...
            history = trainer.train_model(temp_dummy_X, temp_dummy_y) # Passa dummy_y_encoded
            ...
            if history:
                ...
                if 'val_loss' in history.history:
                    ...
                    logger.info(f"Melhor perda de validação: {min(history.history['val_loss']):.4f}")

                if 'val_accuracy' in history.history:

                    logger.info(f"Melhor acurácia de validação: {max(history.history['val_accuracy']):.4f}")

                model_saved_path = trainer.model_path

                if os.path.exists(model_saved_path):

                    logger.info(f"Modelo de teste para {arch} salvo em {model_saved_path}")

                    # ADIÇÃO: Verifica se o LabelEncoder também foi salvo

                    expected_encoder_path = os.path.join(

                        test_model_dir,

                        f"label_encoder_{trainer.timestamp}.pkl"

                    )

                    if os.path.exists(expected_encoder_path):

                        logger.info(f"LabelEncoder correspondente salvo em {expected_encoder_path}")

                    else:

                        logger.warning(f"LabelEncoder correspondente NÃO foi encontrado em {expected_encoder_path}.")

                else:

                    logger.warning(f"Modelo de teste para {arch} NÃO foi salvo em {model_saved_path}.")

            else:

                logger.warning(f"Treinamento para {arch} não foi concluído com sucesso.")
    ...
    if os.path.exists(test_model_dir):
        ...
        logger.info(f"\nRemovendo diretório de teste '{test_model_dir}'...")

        shutil.rmtree(test_model_dir)

        logger.info("Diretório de teste removido.")